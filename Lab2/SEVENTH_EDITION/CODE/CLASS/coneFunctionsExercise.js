...
var nConeSectors = 3;	// Number of sectors in first cone
...
window.onload = function init( ) {
	...
	solidConeBuffers = createCone( nConeSectors2, gl, vec3(1, 1, 1) );
...
function render( ) {
	...
	//adding lighitng
	// var lightAmbient = vec4(0.0, 0.3, 0.0, 1.0 );
	...
	var modelView = lookAt( vec3( 2.0, 1.5, 1.5 ), vec3( 0.5, 0, 0 ), vec3( 0, 1, 0 ) );
	...
	transformation = scalem( 0.5,0.75,0.5 ,0); //mat4( );
    gl.uniformMatrix4fv(vTransformation, false, flatten(transformation));

	// And finally to draw the cone
    //renderCone( buffers, nSectors, gl, program )
	...
	//transformation = mult( translate( 1, -0.5, 0.5 ), scalem( 0.5, 0.5, 0.5 ) );
	//gl.uniformMatrix4fv( vTransformation, false, flatten(transformation));
