//this is not used with the other files
//was trying to incorporate Angels cylinder for the carosaul
//"use strict";

class Cylinder{
    constructor(numSlices, numStacks, gl, program){
        ...
        var stop = bottom + (j+1)*(top-bottom)/this.stacks;
        var sbottom = bottom + j*(top-bottom)/this.stacks;
        var topPoints = [];
        var bottomPoints = [];
        //var topST = [];
        //var bottomST = [];
        for(var i =0; i<this.slices; i++) {
            var theta = 2.0*i*Math.PI/this.slices;
        ...
        for(var i=0; i<this.slices; i++) {
            ...
            cylinderTextureCoordinates.push([(i+1)/this.slices, j*(top-bottom)/this.stacks]);

            cylinderVertexCoordinates.push([b[0], b[1], b[2], 1.0]);
            cylinderVertexColors.push(sideColor);
            cylinderNormals.push([normal[0], normal[1], normal[2]]);
            cylinderTextureCoordinates.push([i/this.slices, (j-1)*(top-bottom)/this.stacks]);

            cylinderVertexCoordinates.push([c[0], c[1], c[2], 1.0]);
            cylinderVertexColors.push(sideColor);
            cylinderNormals.push([normal[0], normal[1], normal[2]]);
            cylinderTextureCoordinates.push([(i+1)/this.slices, (j-1)*(top-bottom)/this.stacks]);
        ...
        for(var i =0; i<this.slices; i++) {
            var theta = 2.0*i*Math.PI/this.slices;
            this.topPoints.push([radius*Math.sin(theta), top, radius*Math.cos(theta), 1.0]);
            bottomPoints.push([radius*Math.sin(theta), bottom, radius*Math.cos(theta), 1.0]);
        };

        this.topPoints.push([0.0, top, radius, 1.0]);
        this.bottomPoints.push([0.0,  bottom, radius, 1.0]);
        return;
    }
    ...
    render(){
        ...
        gl.drawArrays( gl.TRIANGLES, 0, this.topPoints.length);
        gl.drawArrays( gl.TRIANGLES, 0, this.bottomPoints.length);
